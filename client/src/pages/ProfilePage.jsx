...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if(!selectedImg) {
      await update({fullName: name, bio});
      navigate('/');
      return;
    }
    ...
    reader.onload = async () => {
      ...
      await update({profilePic: base64Image, fullName: name, bio});
      navigate('/');
    }
  }
  ...
  return (
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col bg-[url("../assets/bg.jpg")]'>
     <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 
      ...
      <img className={`max-w-44 aspect-square rounded-full mx-10 mx-sm:mt-10 ${selectedImg && 'rounded-full'}`}
